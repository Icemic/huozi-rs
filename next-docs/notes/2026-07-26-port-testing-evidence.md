...
- 日期：2026-07-26
- 状态：`superseded`（历史讨论记录）
- 当前方案：[Huozi × Tiqian 整合 Roadmap](../roadmap.md) · [迭代 01](../iterations/01-tiqian-fixture-parity.md)
...
> 本纪要中的 recorded backend evidence、固定字体数据集和完整跨语言 fixture 工具链不再是现行方案。当前直接复用 Tiqian 的 deterministic-stub fixture 与 checked-in golden，通过临时 exporter 和 adapters 逐用例对照。以下内容仅用于追溯此前讨论。
