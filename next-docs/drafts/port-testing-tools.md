...
- 状态：`superseded`（已作废，不实施）
- 日期：2026-07-26
- 替代方案：[Huozi × Tiqian 整合 Roadmap](../roadmap.md) · [迭代 01](../iterations/01-tiqian-fixture-parity.md)
...
> 本草案提出的固定字体 corpus、record/replay backend、完整 schema、截图和独立 fixture-support 工具链已被后续决策取代。当前直接复用 Tiqian 内部 fixture 与 golden，由临时 exporter、Node.js adapters 和 Rust 自有 JSON 逐用例对照。以下正文仅保留历史背景。
